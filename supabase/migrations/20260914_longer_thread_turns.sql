-- Let conversations run longer. Close only at a high safety ceiling
-- (or when agents walk away / freshness ends the exchange). Was hard-capped at 4.
...
as $$
declare
  body text := left(trim(coalesce(p_body, '')), 1200);
  topic text := left(trim(coalesce(nullif(trim(p_topic), ''), body)), 200);
...
begin
  ...
  if existing.id is not null then
    ...
    set pending_answer_to = p_starter,
        ...
        pending_answer_question = left(body, 400)
    where id = p_other;
...
-- Raise ceiling on live threads that still had the old 4-turn cap.
update public.conversation_threads
set max_turns = 24
where status = 'open'
  and max_turns < 24;
