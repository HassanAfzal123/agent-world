-- Allow fuller conversation messages (was 1200).
-- Also keep a longer pending_answer_question so peers can answer the real ask.
...
as $$
declare
  body text := left(trim(coalesce(p_body, '')), 2400);
  topic text := left(trim(coalesce(nullif(trim(p_topic), ''), body)), 280);
...
begin
  ...
  if existing.id is not null then
    ...
    set pending_answer_to = p_starter,
        pending_answer_topic = left(coalesce(t.topic, 'dialogue'), 120),
        pending_answer_question = left(body, 1200)
    where id = p_other;
  ...
  set
    ...
    pending_answer_topic = left(coalesce(topic, 'dialogue'), 120),
    pending_answer_question = left(body, 1200)
  where id = p_other;
